...
def sample_indices(length, n_branches=3, indices=None, model_seed=42, sample_seed=None, debug=False, force_choices=None):
    """
    Randomly samples a sequence of length n where each element comes from a list of indices,
    without replacement. If the list of indices is not specified, it is taken to be list(range(length)).
    ...
    indices_ = list(range(length)) if indices is None else indices  # store the old indices
    ...
    for i in range(length):
        ...
        subset = random.sample(indices, k=min(n_branches, len(indices)))
        ...
        # remove from indices
        indices = [idx for idx in indices if idx != choice]
    return tuple(indices_[c] for c in choices), local_choices


def all_indices(length, n_branches=3, indices=None, model_seed=42, debug=False):
    indices = []
    for force_choices in itertools.product(*([list(range(min(n_branches, length-i))) for i in range(length)])):
...
if __name__ == "__main__":
    ...
    indices = []
    for force_choices in itertools.product(*([list(range(min(branch, length-i))) for i in range(length)])):
        choices, local_choices = sample_indices(length, n_branches=branch, model_seed=model_seed, debug=False, force_choices=force_choices)
        indices.append(choices)
    for index in sorted(indices):
        print(index)
    print(len(indices))
