...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "token_type_ids", torch.zeros(self.position_ids.size(), dtype=torch.long), persistent=False # type:ignore
        )
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = self.position_ids[:, past_key_values_length : seq_length + past_key_values_length] # type:ignore
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        log_conditional_marginals=None,
        attention_mode=None,
    ) -> Tuple[torch.Tensor]:
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            value_layer = past_key_value[1] # type:ignore
            ...
            key_layer = torch.cat([past_key_value[0], key_layer], dim=2) # type:ignore
            value_layer = torch.cat([past_key_value[1], value_layer], dim=2) # type:ignore
        else:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
        ...
        if self.is_decoder:
            ...
            past_key_value = (key_layer, value_layer) # type:ignore

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2)) # type:ignore

        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            query_length, key_length = query_layer.shape[2], key_layer.shape[2] # type:ignore
            if use_cache:
        ...
        if log_conditional_marginals is not None:
            if len(log_conditional_marginals.size()) == 3: # batch x tgt_seq (tgt) x tgt_seq (competitor)
                log_conditional_marginals = log_conditional_marginals[:, None, :, :].expand(attention_scores.size())

        # Normalize the attention scores to probabilities.
        if attention_mode == "post_scale:albo":
            attention_probs = albo_b(attention_scores, log_conditional_marginals)
        elif attention_mode == "post_scale:mult":
            attention_probs = mult(attention_scores, log_conditional_marginals)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(
            ...
            log_conditional_marginals=log_conditional_marginals,
...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            hidden_states = layer_outputs[0] # type:ignore
            if use_cache:
...
class BertForPreTrainingOutput(ModelOutput):
    ...
    prediction_logits: torch.FloatTensor = None # type:ignore
    seq_relationship_logits: torch.FloatTensor = None # type:ignore
...
class BertModel(BertPreTrainedModel):
    ...
    def _prune_heads(self, heads_to_prune):
        ...
        for layer, heads in heads_to_prune.items():
            self.encoder.layer[layer].attention.prune_heads(heads) # type:ignore
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            self.warn_if_padding_and_no_attention_mask(input_ids, attention_mask) # type:ignore
            input_shape = input_ids.size()
        ...
        extended_attention_mask: torch.Tensor = self.get_extended_attention_mask(attention_mask, input_shape) # type:ignore
