    ...
    def forward(
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            if reencode:
                if "log_conditional_marginals" not in kwargs_encoder: # batch x src_seq x tgt_seq (tgt) x tgt_seq (competitor)
                    raise ValueError("reencode requires conditional marginals")
                log_conditional_marginals = kwargs_encoder["log_conditional_marginals"]
                if len(log_conditional_marginals.size()) != 4:
                    raise ValueError(f"conditional marginals needs to be batch x src_seq x tgt_seq (tgt) x tgt_seq (competitor), got {log_conditional_marginals.size()} instead")
                copies = log_conditional_marginals.size(1)
                kwargs_encoder["log_conditional_marginals"] = log_conditional_marginals.reshape(-1, log_conditional_marginals.size(-1), log_conditional_marginals.size(-1))
                kwargs_encoder["token_type_ids"] = kwargs_encoder["token_type_ids"].repeat_interleave(copies, dim=0)
                kwargs_encoder["position_ids"] = kwargs_encoder["position_ids"].repeat_interleave(copies, dim=0)
                encoder_outputs = self.encoder(
                    input_ids=input_ids.repeat_interleave(copies, dim=0) if input_ids is not None else None,
                    attention_mask=attention_mask.repeat_interleave(copies, dim=0) if attention_mask is not None else None,
                    inputs_embeds=inputs_embeds.repeat_interleave(copies, dim=0) if inputs_embeds is not None else None,
        ...
        if reencode:
            encoder_hidden_states = encoder_outputs[0].reshape(input_ids.size(0), copies * encoder_outputs[0].size(1), encoder_outputs[0].size(2))
        else:
        ...
        if reencode:
            attention_mask = torch.eye(copies, device=input_ids.device).repeat_interleave(input_ids.size(1), dim=1)[None,...].expand(input_ids.size(0), copies, copies * input_ids.size(1))
        # Decode
