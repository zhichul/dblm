...
class ConditionalAutoregressiveInterleavedModel(AutoregressiveInterleavedModel):
    ...
    def sample(self):
        input_ids = torch.stack([self.observation[i] for i in range(self.z0_nvars)] + [torch.ones_like(self.observation[0]).fill_(self.bos_vocab_index)], dim=1)
        past_key_values = None
        for i in range(self.length * 2):
            output = self.transformer(input_ids[:,-1:], past_key_values=past_key_values)
