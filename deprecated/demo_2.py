...
def generate_pgm(nvars=5, nvals=4, mode="random_spanning_tree"):
    if mode == "random_spanning_tree":
        ...
        log_potentials = np.random.rand(
            len(edges), nvals, nvals
        )  # TODO: do a distribution such as gaussian
    return {
        "nodes": vertices,
        "edges": edges,
        "factors": edges,
...
def materialize(pgm):
    ...
    for row in product(*([list(range(nvals))] * nvars)):
        ...
        for i, factor in enumerate(factors):
            ...
            log_potential += log_potentials[(i, *value)]
...
def pgm_sampler(table, nvars, nvals):
    ...
    def sample(n):
        ...
        samples = np.stack(
            [(samples // (nvals ** (nvars - 1 - i))) % nvals for i in range(nvars)],
            axis=1,
        )
        return samples
...
def generate_markov_chain(nstates=5, transition_mode="full"):
    if transition_mode == "full":
        ...
        transition = torch.softmax(
            torch.tensor(np.random.rand(nstates, nstates)), dim=1
        ).numpy()
        initial = torch.softmax(torch.tensor(np.random.rand(1, nstates)), dim=0).numpy()
    return {"transition": transition, "initial": initial}


def chain_sampler(markov_chain):
    transition = markov_chain["transition"]
    initial = markov_chain["initial"]
    transition = torch.distributions.categorical.Categorical(
        logits=torch.tensor(transition)
    )
...
if __name__ == "__main__":
    ...
    sentences = []
    for _, (z0, zis) in enumerate(zip(vsamples, ssamples)):
        sentence = []
        for zi in zis:
            # zi is the index of the variable in z0
            logits = emission[zi](z0[zi])
            emission_dist = torch.distributions.categorical.Categorical(
