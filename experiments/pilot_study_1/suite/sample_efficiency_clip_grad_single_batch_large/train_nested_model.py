...
def main():
    ...
    for seed in [11,12,13,14,15]:
        ...
        assignments = assignments[:2000]
        dev_assignments = dev_assignments[:2000]
        test_assignments = test_assignments[:2000]
        ...
        for epoch in range(nepochs):
            ...
            for offset in bar:
                loss = -nested_model.log_marginal_probability(assignment=[(vi, assignments[example_ids[offset: offset+ batch_size], col]) for col, vi in enumerate(indices)]).mean() / config["sequence_length"]
            ...
            if epoch % 1 == 0:
                # evaluate
                dev_loss = 0 #(-nested_model.log_marginal_probability(assignment=[(vi, dev_assignments[:, col]) for col, vi in enumerate(dev_indices)]).mean() / config["sequence_length"]).item()
                test_loss = 0 #(-nested_model.log_marginal_probability(assignment=[(vi, test_assignments[:, col]) for col, vi in enumerate(test_indices)]).mean() / config["sequence_length"]).item()
