...
def evaluate(model, loader, data_config, no_inference=False, uniform_noise=None):
    loss = 0
    n = 0
    with torch.no_grad():
        model.eval()
...
def main():
    ...
    # load model
    encoder = bert.BertModel(bert.BertConfig(
        vocab_size=data_config["nvars"] * data_config["nvals"] + 1,
        max_position_embeddings=data_config["nvars"],
        num_hidden_layers=0,
        hidden_size=args.n_hidden,
        pad_token_id=data_config["nvars"] * data_config["nvals"]
    ), add_pooling_layer=False)
    decoder = gpt2.GPT2LMHeadModel(gpt2.GPT2Config(vocab_size=data_config["nvars"] * data_config["nvals"] + 1,
    ...
    # evaluate once
    dev_loss = evaluate(model, dev_dataloader, data_config, args.no_inference, uniform_noise=args.uniform_noise)
    test_loss = evaluate(model, test_dataloader, data_config, args.no_inference, uniform_noise=args.uniform_noise)
    logline = {
