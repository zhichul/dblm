...
def parse_args():
    ...
    parser.add_argument("--n_hidden", type=int, required=False)
...
def main():
    ...
    model = gpt2.GPT2LMHeadModel(gpt2.GPT2Config.from_dict(decoder_config))
    model.load_state_dict(torch.load(os.path.join(args.pretrained_model, "pytorch_model.bin")))
    model.to("cuda")
    ...
    if args.multitask_lambda is not None:
        multitask_data = utils.decoder_train_data_generator(data_config["nvars"],
                                                                    data_config["nvals"],
                                                                    data_config["seq_len"],
    ...
    for gpu_batch in bar:
        ...
        if args.multitask_lambda is not None:
            x, px = (i.to("cuda") for i in next(multitask_data)) # type:ignore
            reg_output = model(input_ids=x, position_ids=px, labels=x, return_dict=True)
