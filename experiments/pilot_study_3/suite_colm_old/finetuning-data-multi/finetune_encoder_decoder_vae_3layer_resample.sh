...
do
for N in 500 5000 #5000 #500 5000 50000 #100 250 500 1000 #2500 5000 10000 25000 50000
do
for MULTI_LAMBDA in 1 #1 100 # 0.001 0.01 10000 100000 #
do
OUT_DIR=/${BLU_ARTIFACTS2}/dblm/experiments/pilot_study_3/colm_finetuned_seq2seq_models_vary_data_multi_vae_random_init_resample/${SEED}/${NVARS}/${NVALS}/${SEQ_LEN}/${NBRANCHES}/${X_SEED}/${BATCH_SIZE}/${GPU_BATCH_SIZE}/${LR}/${NLAYER}/${TRAIN_STEPS}/${ZSEED}/${MEAN}/${STD}/${N}/${MULTI_LAMBDA}
...
CUDA_VISIBLE_DEVICES=0 python3 ../../suite/finetuning/finetune_encoder_decoder.py \
    ...
    --no_inference \
    --name Random-Init-VAE-Layer-${NLAYER}-${N}-${MULTI_LAMBDA}-resample-demo \
    --first ${N} \
...
for SEED in 44 46 48 50 # training experiment seed
do
for NVARS in 10
do
for NVALS in 7
...
CUDA_VISIBLE_DEVICES=0 python3 ../../suite/finetuning/finetune_encoder_decoder.py \
    ...
    --group finetune-data-multi-2k \
    --no_inference \
    --name Random-Init-VAE-Layer-${NLAYER}-${N}-${MULTI_LAMBDA} \
    --first ${N} \
    --multitask_lambda ${MULTI_LAMBDA} \
    --random_init \
    --multitask_data_source pilot_study_3_same \
