...
do
for MULTI_LAMBDA in 1 10 100 1000
do
for EVALSTD in 1.1
do
OUT_DIR=${BLU_ARTIFACTS2}/dblm/experiments/pilot_study_3/finetuned_seq2seq_models_with_inference_vary_data_multi_domain_shift_mult_gate_post_scale_gate/${SEED}/${NVARS}/${NVALS}/${SEQ_LEN}/${NBRANCHES}/${X_SEED}/${BATCH_SIZE}/${GPU_BATCH_SIZE}/${LR}/${NLAYER}/${TRAIN_STEPS}/${ZSEED}/${MEAN}/${STD}/${EVALSTD}/${N}/${MULTI_LAMBDA}
...
EVAL_FOLDER="/home/blu/jhu/dblm/experiments/pilot_study_3/data/nvars=${NVARS}-nvals=${NVALS}-zseed=${Z_SEED}-seq_len=${SEQ_LEN}-nbranches=${NBRANCHES}-xseed=${X_SEED}-mean=${MEAN}-std=${EVALSTD}-sseed=${SAMPLE_SEED}-N=100000"
CUDA_VISIBLE_DEVICES=0 python3 ../../suite/finetuning/finetune_encoder_decoder.py \
    --data_config ${DATA_FOLDER}/args.json \
    --data_meta ${EVAL_FOLDER}/meta.json \
    --train_file ${DATA_FOLDER}/train.bin \
    --dev_file ${EVAL_FOLDER}/dev.bin \
    --test_file ${EVAL_FOLDER}/test.bin \
    ...
    --output_dir ${OUT_DIR} \
    --project albo \
    --group finetune-data-multi-domain-shift-${EVALSTD} \
    --name PSG-Mult-Gate-${N}-${MULTI_LAMBDA} \
    --first ${N} \
