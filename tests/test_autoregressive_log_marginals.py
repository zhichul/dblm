...
if __name__ == "__main__":
    ...
    seeding.seed(42)
    transformer = gpt2.GPT2LMHeadModel(gpt2.GPT2Config(vocab_size=24 + 1, n_positions=3 + 1 + 5, bos_token_id=12, n_layer=1, add_cross_attention=True))
    ...
    model = autoregressive_x_models.LatentWorldAutoregressiveXModel(pgmz0.to_factor_graph_model(), pgmxt, transformer.transformer.wte)
    assignment = list(enumerate([ 12, 21, 16, 23, 21])) # 1,  3,  2, 
    model.eval()
    log_marginal = model.log_marginal_probability(assignment)
    breakpoint(
    b = 1
